<template>
  <div class="app-container">
          ...
          <el-table v-loading="loading" :data="availableCoursesList" style="margin-top: 15px;">
            ...
            <el-table-column label="操作" align="center" width="200" class-name="small-padding fixed-width">
              <template #default="scope">
                ...
                <el-button
                  type="info"
                  @click="handleCourseAnalysis(scope.row)"
                >智能分析</el-button>
              </template>
            </el-table-column>
          </el-table>
    ...
    <!-- 课程智能分析对话框 -->
    <el-dialog title="课程智能分析" v-model="analysisDialogVisible" width="800px">
      <div class="analysis-dialog" v-loading="analysisLoading">
        <div v-if="analysisResult" class="analysis-content">
          <div class="analysis-header">
            <el-icon class="analysis-icon"><circle-check /></el-icon>
            <h3>课程智能分析报告</h3>
          </div>
          ...
          <div class="analysis-section">
            ...
            <p class="course-name">{{ analysisResult.courseName }}</p>
          </div>
          
          <div class="analysis-section">
            <h4 class="section-title">
              <el-icon class="section-icon"><chat-line-round /></el-icon>
              AI智能分析 - 先修课程
            </h4>
            <ul class="prerequisite-list">
              <li v-for="(item, index) in analysisResult.prerequisites" :key="index">{{ item }}</li>
            </ul>
          </div>
          
          <div class="analysis-section">
            <h4 class="section-title">
              <el-icon class="section-icon"><arrow-up /></el-icon>
              AI推荐 - 外延拓展课程
            </h4>
            <ul class="prerequisite-list">
              <li v-for="(course, index) in analysisResult.extensionCourses" :key="index">{{ course }}</li>
            </ul>
          </div>
          ...
          <div class="analysis-section">
            ...
            <ul class="learning-advice-list">
              <li v-for="(advice, index) in analysisResult.learningAdvice" :key="index">{{ advice }}</li>
            </ul>
          </div>
          ...
          <div class="analysis-section">
            ...
            <div class="tags">
              <el-tag v-for="(tag, index) in analysisResult.tags" :key="index" :type="getTagType(index)">
                {{ tag }}
              </el-tag>
            </div>
          </div>
          ...
          <div class="analysis-footer">
            <el-tag size="small" type="info">DeepSeek AI生成</el-tag>
          </div>
        </div>
...
import { Search, Refresh, Loading, Star, ChatLineRound, ArrowUp, Check, Ticket, CircleCheck } from '@element-plus/icons-vue'
...
async function analyzeCourse(course) {
  try {
    ...
    const openai = new OpenAI({
      ...
      apiKey: 'sk-943f5070ba7344d8a5dd73aa9d561b25', // 替换为真实的DeepSeek API密钥
    ...
    const completion = await openai.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: '你是一个专业的课程分析助手，需要根据课程信息提供详细的分析报告。请按照以下格式输出：\n\n# 课程智能分析报告\n\n## 课程名称\n[课程名称]\n\n## 先修课程\n- [先修课程1]\n- [先修课程2]\n- [先修课程3]\n\n## 外延拓展课程\n- [拓展课程1]\n- [拓展课程2]\n- [拓展课程3]\n\n## 学习建议\n[学习建议内容]\n\n## 相关标签\n[标签1], [标签2], [标签3], [标签4]'
        },
        {
          role: 'user',
          content: `请分析以下课程：\n课程名称：${course.courseName}\n课程编码：${course.courseCode}\n课程类型：${course.courseType}\n学分：${course.credits}\n课程描述：${course.description || '暂无描述'}`
        }
      ],
      model: 'deepseek-chat',
      temperature: 0.7,
    ...
    // 解析API返回的内容，提取分析结果
    const analysis = parseAnalysisResult(analysisContent, course.courseName);
    analysisResult.value = analysis;
    
    // 保存分析结果到本地存储
    saveAnalysis(course.courseCode, analysis);
  } catch (error) {
    console.error('课程分析失败:', error);
    // API调用失败时，显示错误信息，不使用模拟数据
    analysisResult.value = {
      courseName: course.courseName,
      prerequisites: [`分析失败：${error.message || '未知错误'}`],
      extensionCourses: ['请检查API密钥是否正确'],
      learningAdvice: ['分析服务暂时不可用，请稍后再试。'],
      tags: ['分析失败']
    };
  } finally {
...
/** 解析分析结果 */
function parseAnalysisResult(content, courseName) {
  // 解析豆包API返回的Markdown格式内容
  const result = {
    courseName,
    prerequisites: [],
    extensionCourses: [],
  ...
  // 提取先修课程
  const prerequisiteMatch = content.match(/## 先修课程[\s\S]*?(?=##|$)/)
  if (prerequisiteMatch) {
    const lines = prerequisiteMatch[0].split('\n')
    for (const line of lines) {
      if (line.trim().startsWith('-')) {
        result.prerequisites.push(line.trim().substring(2).trim())
      }
    }
  }
  
  // 提取外延拓展课程
  const extensionMatch = content.match(/## 外延拓展课程[\s\S]*?(?=##|$)/)
  if (extensionMatch) {
    const lines = extensionMatch[0].split('\n')
    for (const line of lines) {
      if (line.trim().startsWith('-')) {
        result.extensionCourses.push(line.trim().substring(2).trim())
      }
    }
  }
  
  // 提取学习建议
  const adviceMatch = content.match(/## 学习建议[\s\S]*?(?=##|$)/)
  if (adviceMatch) {
    const adviceContent = adviceMatch[0].replace('## 学习建议', '').trim()
    // 移除 ** 符号
    const cleanedContent = adviceContent.replace(/\*\*/g, '')
    // 分割为数组，按数字编号或换行符分割
    const adviceLines = cleanedContent.split(/\n\s*\d+\.\s*/).filter(line => line.trim())
    if (adviceLines.length > 0) {
      result.learningAdvice = adviceLines
    } else {
      // 如果没有数字编号，按换行符分割
      const lines = cleanedContent.split('\n').filter(line => line.trim())
      result.learningAdvice = lines
    }
  }
  
  // 提取相关标签
  const tagsMatch = content.match(/## 相关标签[\s\S]*?(?=##|$)/)
  if (tagsMatch) {
    const tagsLine = tagsMatch[0].split('\n')[1]?.trim()
    if (tagsLine) {
      result.tags = tagsLine.split(',').map(tag => tag.trim())
    }
  }
  
  // 确保所有字段都有值
  if (result.prerequisites.length === 0) {
    result.prerequisites = ['暂无先修课程要求']
  }
  
  if (result.extensionCourses.length === 0) {
    result.extensionCourses = ['暂无推荐拓展课程']
  }
  
  if (result.learningAdvice.length === 0) {
    result.learningAdvice = ['暂无学习建议']
  }
...
/** 处理课程智能分析 */
function handleCourseAnalysis(course) {
  analysisDialogVisible.value = true
  analysisLoading.value = true
  
  // 检查本地存储中是否有该课程的分析结果
  const savedAnalysis = getSavedAnalysis(course.courseCode)
  if (savedAnalysis) {
    // 如果有保存的结果，直接使用
...
/** 从本地存储获取保存的分析结果 */
function getSavedAnalysis(courseCode) {
  const studentId = userStore.userId
  const key = `analysis_${studentId}_${courseCode}`
  const savedData = localStorage.getItem(key)
  return savedData ? JSON.parse(savedData) : null
}

/** 保存分析结果到本地存储 */
function saveAnalysis(courseCode, analysis) {
  const studentId = userStore.userId
  const key = `analysis_${studentId}_${courseCode}`
