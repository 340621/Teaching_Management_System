<template>
  <div class="app-container">
    ...
    <!-- 课程资料对话框 -->
    <el-dialog title="课程资料" v-model="materialsDialogVisible" width="800px">
      <el-table v-loading="materialsLoading" :data="courseMaterials">
...
function handleMaterials(row) {
  ...
  getCourseMaterialsList(row.courseId)
}

/** 获取课程资料列表 */
function getCourseMaterialsList(courseId) {
  ...
  getCourseMaterials(courseId).then(response => {
    ...
    ElMessage.error('获取课程资料列表失败: ' + (error.message || '未知错误'))
    courseMaterials.value = []
    materialsLoading.value = false
  })
}
